...
export function CategorySection({ category, onUpdateCategory, onDeleteCategory, onAddMod, onUpdateMod, onDeleteMod }: Props) {
  const [open, setOpen] = useState(true);
  const [editingName, setEditingName] = useState(false);
  const [nameInput, setNameInput] = useState(category.name);
  const [addingMod, setAddingMod] = useState(false);
  const [form, setForm] = useState<AddModForm>({ name: '', priceMin: '', priceMax: '', url: '' });

  const color = getCategoryColor(category.name);

  const totalLow = category.mods.reduce((sum, m) => sum + calcTotal(m.price_min, m.price_max).low, 0);
  const totalHigh = category.mods.reduce((sum, m) => sum + calcTotal(m.price_min, m.price_max).high, 0);
  const categoryTotal = formatPrice(totalLow, totalHigh === totalLow ? totalLow : totalHigh);
    ...
    <div className="bg-[#111111] border border-[#1e1e1e] rounded-xl overflow-hidden">
      ...
      >
        ...
        {editingName ? (
          ...
          <span className="flex-1 text-white font-semibold text-sm tracking-wide">{category.name}</span>
        )}

        <div className="flex items-center gap-2 ml-auto">
          <span className="text-xs font-mono text-gray-400">{categoryTotal}</span>
          {editingName ? (
            <div className="flex gap-1" onClick={e => e.stopPropagation()}>
              <button onClick={() => setEditingName(false)} className="text-gray-500 hover:text-gray-300 p-0.5"><X size={13} /></button>
              <button onClick={saveName} className="text-green-500 hover:text-green-400 p-0.5"><Check size={13} /></button>
            </div>
          ) : (
      ...
      {open && (
        <div className="border-t border-[#1a1a1a] px-2 py-1">
          {category.mods.length === 0 && !addingMod && (
            <p className="text-gray-600 text-xs text-center py-3 italic">No parts yet</p>
          )}
          ...
          {addingMod && (
            <div className="bg-[#1a1a1a] border border-[#2a2a2a] rounded-lg p-3 mt-2 space-y-2">
              ...
              <div className="flex gap-2 justify-end">
                <button onClick={() => setAddingMod(false)} className="flex items-center gap-1 text-xs text-gray-400 hover:text-white px-3 py-1.5 rounded-md border border-[#333] transition-colors">
                  <X size={12} /> Cancel
                </button>
                <button onClick={submitMod} className="flex items-center gap-1 text-xs text-white bg-red-700 hover:bg-red-600 px-3 py-1.5 rounded-md transition-colors">
